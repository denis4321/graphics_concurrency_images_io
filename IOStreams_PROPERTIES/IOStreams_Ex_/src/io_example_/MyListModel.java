...
public class MyListModel implements ListModel, Serializable{
    ...
    public void save4() throws IOException{
        ...
        for (Element e : data){
            ...
            //props.setProperty(index+".val", e.someVal+"");//Это некарсивая ;( конструкция
            index++;
        }
    ...
    public void load4(InputStream fin) throws FileNotFoundException, IOException {
        ...
        int index = 0;
        String name = null;
        while ((name=props.getProperty(index+".name"))!=null){
            defs.setProperty(index+".val", "123");
            long time = Long.parseLong(props.getProperty(index+".time"));
            Element elem = new Element(name);
            elem.setTime(time);
            //int val = Integer.parseInt(props.getProperty(index+".val", "123"));//Мы поменяли формат
            //int val = Integer.parseInt(props.getProperty(index+".val"));
            data.add(elem);
            index++;
        }
        this.fireContentsChanged(this, 0, Math.max(old, data.size()));//кидает событие изменения данных
