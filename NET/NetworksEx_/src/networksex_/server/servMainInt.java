...
public class servMainInt extends JFrame implements ConnectionListener {
    public File DATA = new File(
            "C:\\Documents and Settings\\Me\\–‡·Ó˜ËÈ ÒÚÓÎ\\data.txt");
    ...
    public void save(Connection c) {
        ...
        try {
            ...
            for (Person1 ppp : data) {
                ...
                pr.setProperty("# " + i + "  " + c.getAddress(),
                               "Consumered" + ppp.getName());
                pr.store(out, "");
                i++;
            }
        } catch (Exception ex1) {
            ...
            if (currC != c) {
                ...
                if (currC.getKindOfMesaage()) {
                    try {
                        Thread.sleep(1001);
                        currC.send("¬¿ÿ «¿ ¿« œ–»Õﬂ“");
                        currC.listener.connectionPause(currC);
                        save(currC);
                    } catch (Exception ex1) {
                        ex1.printStackTrace();
                    }
